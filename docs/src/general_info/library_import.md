...
Open the volume, choose **Manage Files**, and press **Keep Unmatched**. The window says how many of the folder's files are matched to nothing, and the button carries that count. Every file in the folder that nothing is currently matched to is recorded against the volume as an `adopted` general file. The files do not move or get renamed. They stop being offered for import, and every later file scan skips them.
...
When ComicVine does not have a folder's series, Kapowarr asks the other configured providers -- the Grand Comics Database and Metron -- before holding it for review. It stops at the first provider that offers a candidate the matcher can actually use, rather than at the first provider that merely returns a row with the right title: those are different tests, and stopping on the weaker one meant a folder could be held for review with two databases never consulted. A series the default provider can serve therefore still costs the single request it always did, and the extra requests only happen for folders that were going to be held anyway.

Review records say which providers were asked, what each returned, and which one was never reached -- a provider that answers with nothing leaves no candidates behind, so counting candidates alone cannot tell "nobody has this" from "we stopped before asking".
