...
class NZBDownload(ExternalDownload, BaseDirectDownload):
    "For downloading via a Usenet client (e.g. SABnzbd)"
    ...
    def __init__(
        ...
        if not self._filename_body:
            # Unlike a magnet link (whose torrent name is resolvable
            # before the download starts via magnet2torrent.com, see
            # TorrentDownload), an NZB job's real name is decided by
            # SABnzbd/the indexer and isn't known here -- fall back to
            # the search result's own title.
            self._filename_body = splitext(web_title or 'unknown')[0]

        self._title = basename(self._filename_body)
        # The real download folder isn't known until the external client
        # reports the job done (see SABnzbd.get_download()'s 'storage'
        # key) -- update_status() fills this in once available. Nothing
        # reads `files` before the download reaches IMPORTING_STATE.
    ...
    def update_status(self) -> None:
        ...
        # Client-specific extra: the folder the client (e.g. SABnzbd)
        # ended up storing the completed download in. Not every
