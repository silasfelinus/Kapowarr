...
2. ~~**Watched-folder / external completed-download import**~~ — *shipped.* `Settings → Download → Watched Folder` names an inbound folder that a 15-minute task scans. A file is imported once it has stopped changing and matches exactly one volume already in the library; unmatched, ambiguous and still-writing files are left untouched. Remaining follow-ups: surface unmatched files in the manual-import workbench rather than only in the task message, and let a watched folder feed volume *creation* once that can share Continuous Library Import's ComicVine pacing.
