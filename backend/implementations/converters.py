...
def conversion_failed(
    ...
    source. `rar_to_zip` extracted, zipped and then deleted the original
    without ever checking that the extraction worked -- and `unrar`
    failing leaves an empty folder, which zips to a valid 22-byte archive
    holding nothing. The record was repointed at that stub and the `.cbr`
    deleted, so the issue counted as complete and the comic was gone.
    Silas's library had twenty of them on 2026-09-07, one of which
    (`Adam Strange 01 (2004).cbz`) had a perfectly good 16 MB original
    still sitting in the download folder being re-fetched.
    ...
    """
    LOGGER.error(
        "Could not convert %s%s. Leaving the original alone -- a "
        "conversion that fails must not cost the file it was given.",
